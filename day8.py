...
# Part 1
#
# Given a set of instructions of the form '<reg> (inc|dec) <intval> if <reg> (==|!=|>|>=|<|<=) <intval>'
# Process each instruction, where registers get initialized to 0 the first time they are referenced
# Return the maximum value of any register after all instructions are processed
#
# My solution:
#   Initialize `registers` hash to {}
#   Process each instruction in turn
#   Use a regex to parse each instruction
#   Return the maximum value in the `registers` hash

    def part1(self):
        registers = {}
        re_incdec = re.compile('^ *(?P<reg>[^ ]+) +(?P<operator>inc|dec) +(?P<operand>-?[0-9]+) +if +(?P<condreg>[^ ]+) +(?P<cond>==|!=|>|<|>=|<=) +(?P<comparator>-?[0-9]+) *$')
        conditions = {
            '==': lambda a,b: a==b ,
            '!=': lambda a,b: a!=b ,
            '>' : lambda a,b: a>b  ,
        ...
        operations = { 
            'inc': lambda a,b: registers.update([(a, registers.get(a,0) + b)]) ,
            'dec': lambda a,b: registers.update([(a, registers.get(a,0) - b)]) ,
        }
        for line in self.program_input:
            incdec = re_incdec.match(line)
            if incdec:
                incdec = incdec.groupdict()
                condreg = registers.get(incdec['condreg'], 0)
                comparator = int(incdec['comparator'])
                cond = conditions[incdec['cond']](condreg, comparator)
                if cond:
                    operations[incdec['operator']](incdec['reg'], int(incdec['operand']))
            elif len(line) > 0:
                print "Unparsed instruction! {0!r}".format(line)
        return max(registers.values())

#
# Part 2
#
# Same as part 1, except keep track of `maxval`
#
# My solution:
#   (Pseudocode)
    ...
    def part2(self):
        ...
        for line in self.program_input:
            ...
            if incdec:
                ...
                if cond:
                    operations[incdec['operator']](incdec['reg'], int(incdec['operand']))
                    maxval = max(maxval, registers[incdec['reg']])
            elif len(line) > 0:
                print "Unparsed instruction! {0!r}".format(line)
        return maxval
...
day8 = Day8(puzzle_input)

print 'part 1 solution = {0}'.format( day8.part1() )
print 'part 2 solution = {0}'.format( day8.part2() )
