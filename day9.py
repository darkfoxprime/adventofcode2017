...
#
# Definitions:
#   Garbage - any sequence of characters beginning with `<`
#       and ending with `>`.  Within garbage, a ! cancels
#       the next character (so it has no meaning).
#   Group - A sequence of groups and/or garbage beginning with `{`
#       and ending with `}`.
#   Score - A group's score is 1 higher than its containing group.
#       The outermost group has a score of 1.
#
# Calculate the total score of all the groups in the stream.
#
# My solution:
#   State machine:  The `state` is (flag,score)
#   Initialize state to ('{',0)
#   Initialize total to 0.
#   For each character in the input stream:
#     Flag '{' - in group
#       If character is '{', increment score.
#       If character is '<', set flag to '<'
#       If character is '}', add score to total, decrement score, and set flag to ','
#     Flag ',' - in group after item
#       If character is ',', set flag to '{'
#       If character is '}', add score to total and decrement score.
#     Flag '<' - in garbage
#       If character is '>'> set flag to ','
#       If character is '!', set flag to '!'
#     Flag '!' - in garbage, skipping character
#       Set flag to '<'
#   Assertion: At end of input, state should be (',',0)
#   Return total.

    def part1(self):
        total = 0
        state = ( '{', 0 )
        for char in self.program_input:
            #print "{0!r} {1!r}".format(state, char)
            assert state[0] in ('{', ',', '<', '!'), "Invalid state {0!r}".format(state)
            if state[0] == '{':
                assert char in ('{', '}', '<'), "Invalid group character {0!r}".format(char)
                if char == '{':
                    state = ( '{', state[1] + 1 )
                elif char == '<':
                    state = ( '<', state[1] )
                ...
                assert char in (',', '}'), "Invalid after-item character {0!r}".format(char)
                if char == '}':
                    total += state[1]
                    state = ( ',', state[1] - 1 )
                elif char == ',':
                ...
                # no assertion; character can be anything
                if char == '>':
                    state = ( ',', state[1] )
                elif char == '!':
                    state = ( '!', state[1] )
            elif state[0] == '!':
                # ignore character entirely
                state = ( '<', state[1] )
        assert state == ( ',', 0 ), "Invalid final state {0!r}".format(state)
        return total
...
#
# My solution:
#   Same basic state machine as part 1.
#   State machine:  The `state` is (flag,score)
#   Initialize state to ('{',0)
#   Initialize total to 0.
#   For each character in the input stream:
...
#       If character is '{', increment score.
#       If character is '<', set flag to '<'
#       If character is '}', decrement score, and set flag to ','
#     Flag ',' - in group after item
#       If character is ',', set flag to '{'
#       If character is '}', d decrement score.
#     Flag '<' - in garbage
#       If character is '>'> set flag to ','
#       If character is '!', set flag to '!'
