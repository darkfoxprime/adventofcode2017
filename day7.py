...
class Day7(object):
    ...
    def __init__(self, program_input):
        self.program_input = program_input.strip('\n').split('\n')
...
#
# What is the bottom program?
#
# My solution:
#   Initialize two sets: `all`, `held`
#   For each line in the puzzle input:
#       Parse the line into the `holder`, `weight`, and `balancers` using a regex
#       Split `balancers` using another regex
#       Add `holder` to `all` and `balancers` to `held`.
#   At the end, return the difference between `all` and `held`.

    def part1(self):
        all = set()
        held = set()
        re_line = re.compile('^(?P<holder>[a-z]+) +\((?P<weight>[0-9]+)\)(?: +-> +(?P<balancers>[a-z]+(?:, +[a-z]+)*))?$')
        re_split = re.compile(', *')
        for line in self.program_input:
            line = re_line.match(line).groupdict()
            line['balancers'] = re_split.split(line['balancers']) if line['balancers'] else []
            all.add(line['holder'])
            held.update(set(line['balancers']))
        return tuple(all - held)
...
#
# My solution:
#   Use the regex's from part 1 to transform the program input to a
#               list of (program, weight, (balancers...)) entries.
#   Initialize `stacks` to an empty hash.
#   Loop:
#       Pop the first item off the front of the list.
#       If any of its balancers are not defined in `stacks`, push it
#                   back to the end of the list, and restart the loop.
#       Check if any of the balancers' `total_weight` is different
#                   than the others.
#           If so, determine the weight needed for the off-weight
#                       balancer in order to make it match.
#           Return that weight./
#       Calculate the program's `total_weight` (its weight plus the
#                   sum of the `total_weight`s of its balancers).
#       Record in `stacks` the program's weight and total_weight:
#                   stacks[program] = (program, weight, total_weight)
    ...
    def part2(self):
        ...
        programs = [
                    (
                      ...
                      , int(line['weight'])
                      , tuple(
                            re_split.split(line['balancers'])
                            if line['balancers'] 
                            else []
                        )
                    )
                    for line in [
                        re_line.match(line).groupdict()
                        for line in self.program_input
                    ]
                ]
        stacks = {}
        while len(programs) > 0:
            ...
            balancers = [stacks.get(balancer, None) for balancer in program[2]]
            if None in balancers:
                programs.append(program)
            else:
                balancers = sorted(balancers, key=lambda b:b[2])
                if len(balancers) > 0 and balancers[0][2] != balancers[-1][2]:
                    if balancers[0][2] != balancers[1][2]:
                        bad = balancers[0]
                    else:
                        bad = balancers[-1]
                    new_weight = bad[1] + balancers[1][2] - bad[2]
                    return (bad[0], bad[1], new_weight)
                stacks[program[0]] = (program[0], program[1], program[1] + sum(b[2] for b in balancers))
        return None
...
day7 = Day7(puzzle_input)

print 'part 1 solution = {0}'.format( day7.part1() )
print 'part 2 solution = {0}'.format( day7.part2() )
