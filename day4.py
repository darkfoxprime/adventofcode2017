...
#
# My solution:
#   Convert puzzle input to a list (strip newlines from beginning and end, split on newlines)
#   Convert list of passphrases to list of words (split each line on any whitespace)
#   Reduce list of passphrases to a count of valid passphrases:
#     Initialize accumulator to 0
#     If passphrase is valid, reduce to accumulator + 1, else reduce to accumulator
#     passphrase is valid if len(set(passphrase)) == len(passphrase)

    def part1(self):
        return reduce(
                    lambda count,passphrase:
                        count + 1 if len(set(passphrase)) == len(passphrase) else count
                  , (line.split() for line in (self.program_input.strip().split('\n')))
                  , 0
                )
...
#
# My solution:
#   Same as for part 1, with an additional conversion step:
#   Convert a list of passphrase words to a set of letter counts:
#     'foo' becomes set([('f',1),('o',2)])
#     'bar' becomes set([('a',1),('b',1),('r',1)])
#     Use `reduce` on the sorted word:
#       Initialize accumulator to ((sorted(word)[0], 1),)
#       Reduce across sorted(word)[1:]
#       If current letter = accumulator[-1][0],
#         return accumulator[:-1] + ((accumulator[-1][0], accumulator[-1][1]+1),)
#       else
#         return accumulator + ((letter,1),)
        ...
        return reduce(
                  ...
                  , (
                        [
                            reduce(
                                lambda accum,letter:
                                    accum + ((letter,1),) if letter != accum[-1][0]
                                    else accum[:-1] + ((accum[-1][0], accum[-1][1]+1),)
                              , sorted(word)[1:]
                              , ((sorted(word)[0],1),)
                            )
                            for word in line.split()
                        ]
                        for line in (self.program_input.strip().split('\n'))
                    )
                  , 0
                )
...
day4 = Day4(puzzle_input)

print 'part 1 solution = {0}'.format( day4.part1() )
print 'part 2 solution = {0}'.format( day4.part2() )
